...
This page describes some weird moments and caveats about C++. Some of them can lead to a disaster.
...
- **gvalue**: has identity - `int& f()` or `int&& f()`
...
SHolder holder("CONST STR", "NON-CONST STR");
```
...
Yes, we can utilize something like boost::noncopyable. But there are "rare" cases when those are not possible.
These "rare" cases are exactly the reasons for having these rules.
